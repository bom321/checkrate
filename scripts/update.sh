...
# ── **ห้ามพยายามหยุดคอนเทนเนอร์ผ่าน API ของ DSM (`synowebapi`) เพื่อกันแจ้งเตือน** ──
# ลองแล้วบนเครื่องจริง ส.ค. 2569 แล้วถอนออก (v1.1.6 → v1.1.7) — เหตุผลเต็มพร้อมผลที่วัดได้อยู่ใน
# DEPLOY.md หัวข้อ "แจ้งเตือน stopped unexpectedly" ย่อ ๆ: Container Manager แปลชื่อคอนเทนเนอร์เป็น
# container ID จากฐานข้อมูลของตัวเอง **และไม่ sync ให้เองถ้าไม่มีคนเปิดหน้า Container Manager**
# ขณะที่ compose สร้าง ID ใหม่ทุกครั้งที่ recreate → ยิงจาก Task Scheduler ได้ `No such container`
# ทุกรอบ กันแจ้งเตือนไม่ได้เลย · วิธีที่ใช้จริงคือปิด event นั้นใน Control Panel → Notification
# ซึ่งไม่ต้องแตะสคริปต์นี้เลย
